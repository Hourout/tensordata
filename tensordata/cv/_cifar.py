...
def cifar10(root):
    ...
    task_path = os.path.join(root, 'cifar10')
    ...
    with tarfile.open(os.path.join(task_path, url.split('/')[-1])) as t:
        t.extractall(task_path)
    noise_flie = tf.gfile.ListDirectory(task_path)
    for file in ['data_batch_1.bin', 'data_batch_2.bin', 'data_batch_3.bin', 'data_batch_4.bin', 'data_batch_5.bin']:
        with open(os.path.join(task_path, file), 'rb') as fin:
    ...
    for file in noise_flie:
        tf.gfile.Remove(os.path.join(task_path, file))
...
def cifar100(root, fine_label=True):
    ...
    start = time.time()
    assert tf.gfile.IsDirectory(root), '`root` should be directory.'
    task_path = os.path.join(root, 'cifar100')
    ...
    with tarfile.open(os.path.join(task_path, url.split('/')[-1])) as t:
        t.extractall(task_path)
    noise_flie = tf.gfile.ListDirectory(task_path)
    with open(os.path.join(task_path, 'train.bin'), 'rb') as fin:
    ...
    for idx in range(train.shape[0]):
        imageio.imsave(os.path.join(task_path, 'train', str(train_label[idx]), str(idx)+'.png'), train[idx])
    with open(os.path.join(task_path, 'test.bin'), 'rb') as fin:
        data = np.frombuffer(fin.read(), dtype=np.uint8).reshape(-1, 3072+2)
